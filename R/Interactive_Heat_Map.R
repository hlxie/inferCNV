...
# Create highly interactive heat maps for single cell expression data using 
...
#
# Args: 
#	plot_data: Expression data
#	reference_idx: (vector) vector of all referecne cell line ID's  
#   ref_index: (list) list of index values for each reference group  
#	ref_groups: (list) list of cell ID's for each reference groups
#	location_data: (data frame) Data frame with gene locations 
#	path_to_shaidyMapGen: (string) Path to the location of ShadyMapGen.jar
#	out_dir: (string) Path to where output should be placed 
#   contigs: (list) List of the chr in order 
#	title: (string) Title that will be used for the heatmap 
# 	gene_symbal: (string) Specify the label type that is given to the gene needed to create linkouts, default is NULL
#
# Returns:
#	Writes a NGCHM file (.ngchm)
# 
# Requires:
#	NGCHM, tidyverse, dplyr, ape, RcolorBrewer 
#
Create_NGCHM <- function(plot_data,
    ...
    if (!file.exists(path_to_shaidyMapGen)){
        stop("Error: The pathway \"", path_to_shaidyMapGen,"\" does not exist.")
    } else {
        shaidy.path <- unlist(strsplit(path_to_shaidyMapGen, 
    ...
    location_data <- rownames_to_column(location_data, var = "Gene")
    ...
    chr <- 
        ...
        as.character() 
    ## get the gene ID's 
    names(chr) <- gene_locations %>% pull(Gene)
    ...
    if (length(ref_groups) > 1) {
        for(i in 1:length(ref_groups)){ 
            cell_type <- replace(cell_type, 1:length(cell_type) %in% ref_index[[i]], paste("Reference",toString(i),sep = "")) 
        }
    } else {
        for(i in 1:length(ref_groups)){ 
    ...
    message(paste0("Saving new NGCHM object"))
